...
export default function ExpandSidebar({ items = [], mobile = false, onNavigate }) {
  ...
  const width = isExpanded ? 240 : 0;

  return (
    <aside
      style={{ width }}
      className="fixed top-0 left-[72px] h-screen transition-all duration-300 overflow-hidden z-30"
    >
      <div className="h-full backdrop-blur-xl bg-black/70 border-r border-white/10 p-4">
        {items.map((i) => (
          <Link
            ...
            className={`flex gap-3 px-3 py-2 rounded-lg mb-2 ${
